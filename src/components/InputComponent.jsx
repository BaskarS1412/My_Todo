...
const InputComponent = ({ addTodoData }) => {
  ...
  const Update = () => {
    setDisplayState((prevcard) =>
      prevcard.map((card) =>
        card.id === EditCard.id
          ? { ...card, name: NameInput, description: DescriptionInput }
          : card
      )
    );
    setEditCard(null);
  ...
  const handleCompletionChange = (id, completionStatus) => {
    const changedCards = DisplayState.map((card) =>
      card.id === id ? { ...card, completed: completionStatus } : card
    );
  ...
  const sortedCards =
    overallStatusFilter === "All"
      ? DisplayState
      : DisplayState.filter((card) => card.completed === overallStatusFilter);
